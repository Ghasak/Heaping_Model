...
#########################################################################
# Based on the code by ??douard Tallent @ TaGoMa.Tech
# September 2012
...
z <- outer(x,y,bivariate) # joint
fx <- rowSums(z) # marginal X
fy <- colSums(z) # marginal Y

# Plot
persp(x, y, z, xlim = c(-3,3), ylim = c(-3,3), main = "Bivariate Normal Distribution",
      sub = bquote(bold(mu[1])==.(mu1)~", "~sigma[1]==.(sig1)~", "~mu[2]==.(mu2)~
                     ", "~sigma[2]==.(sig2)~", "~rho==.(rho)),
      col="orchid2", theta = persp.theta, phi = persp.phi, r = 40, d = 0.1, expand = 0.5,
      ltheta = 90, lphi = 180, shade = 0.4, ticktype = "detailed", nticks=5)

#rotate.persp(x, y, z)

# Plot Marginal X
#plot(x, fx, type="l", main = "Marginal (X) Normal Distribution")

# Plot Marginal Y
#plot(y, fy, type="l", main = "Marginal (Y) Normal Distribution")
